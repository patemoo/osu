...
fn main() {
    ...
    // Push thread results to intermediate_sums
    ...
    // Create Arc Mutex for partition vectors
    let arc_xs : Arc<Mutex<Vec<Vec<usize>>>> = Arc::new(Mutex::new(xs2));

    // Spawn threads per the number passed as partition qty arg.
    for i in 0..num_partitions {
        // Call the clone method of Arc to create a ponter for each partition.
        let arc_x = Arc::clone(&arc_xs);
    ...
    for handle in handles {
        intermediate_sums2.push(handle.join().unwrap());
    }
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    // Get extra elements count.
    let mut extra = v.len() % num_partitions;

    // Get partition_size.
    let partition_size = v.len() / num_partitions;
    let mut start_index = 0;
    let mut end_index = partition_size;

    // Loop to create partitions.
    for _i in 0..num_partitions {
        // Create a vector of integers.
        let mut x : Vec<usize> = Vec::new();
        // Increase end index if there are extra elements still to be added.
        if extra > 0 {
            end_index += 1;
        }
        // Add the current set of integers.
        for j in start_index..end_index {
            x.push(v[j]);
        }
        // Adjust the start index if end index was increased.
        if extra > 0 {
            start_index += 1;
            extra -= 1;
        }
        // Set index values for the next partition.
